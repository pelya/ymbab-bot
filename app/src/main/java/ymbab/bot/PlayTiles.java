...
public class PlayTiles {
    ...
    public static abstract class Tile {
        ...
        void get(TileRegion t, float x, float y) {
            r.set(t.image.getPlanes()[0].getBuffer().getInt((t.y + (int)(t.h * y)) * t.image.getPlanes()[0].getRowStride() + (t.x + (int)(t.w * x)) * 4));
            if (t.debug > 0) {
                t.debug --;
                ...
                t.draw.drawText(r.r + "," + r.g + "," + r.b, t.x + t.w / 2, t.y + t.h * (8 - t.debug) / 4, debugTextSmall);
            }
        }
    }
